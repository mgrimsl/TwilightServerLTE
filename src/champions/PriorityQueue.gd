...
class PriorityQueue:
	...
	func get_item_priority(arrayIdx):
		...
		if (arrayIdx + 1 > currentSize || arrayIdx < 0):
			...
			print(": PRIORITY QUEUE (" + self.name + "): get_item_priority(arrayIdx) out of bounds! arrayIdx = " + str(arrayIdx) + ", currentSize = " + str(currentSize))
			return false
	...
	func insert(priority, value):
		...
		if (typeof(priority) != TYPE_INT):
			...
			print(": PRIORITY QUEUE (" + self.name + "): Priority of incorrect type (" + str(typeof(priority)) + ") (should be float or int)! Aborting insert()")
			return false
	...
	func remove_root(return_array = false):
		...
		if (isEmpty):
			...
			print(": PRIORITY QUEUE ("+self.name+"): Can't delete root in empty heap! isEmpty = true, currentSize = " + str(currentSize))
			return null
	...
	func change_priority(arrayIdx, new_priority):
		...
		if (arrayIdx < 0 || arrayIdx + 1 > currentSize):
			...
			print(": PRIORITY QUEUE (" +self.name+ "): Can't access array at index '" + str(arrayIdx) + "', out of bounds!")
			return false
		
		if (typeof(new_priority) != TYPE_INT):
			
			# Invalid priority parameter. Abort
			print(": PRIORITY QUEUE (" +self.name+ "): Can't assign priority '" + str(new_priority) + "', NaN!")
			return false
	...
	func remove(arrayIdx):
		...
		if (arrayIdx + 1 > currentSize || arrayIdx < 0):
			...
			print(": PRIORITY QUEUE ("+self.name+"): remove(arrayIdx) out of bounds! arrayIdx = " + str(arrayIdx) + ", currentSize = " + str(currentSize))
			return null
	...
	func merge_with(priority_queue, set_maintain_min = true):
		...
		var queue = PriorityQueue.new(list, set_maintain_min)
